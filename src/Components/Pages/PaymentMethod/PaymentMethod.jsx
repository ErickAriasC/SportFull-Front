...
const PaymentMethod = () => {
  ...
  const fetchTotalValue = async () => {
    try {
      const response = await fetch(`https://sportfull-backend-production.up.railway.app/reservas/${reservaId}/valorTotal`);
      if (!response.ok) {
  ...
  const handleFullScreenPayment = () => {
    ...
    const paymentData = {
      ...
      lang: "es",
      external: "false",
      response: `https://sportfull-backend-production.up.railway.app/api/pagos/respuesta?reservation_id=${reservaId}`,
      confirmation: `https://sportfull-backend-production.up.railway.app/api/pagos/confirmacion?reservation_id=${reservaId}`,
      method: "popup",
    };
